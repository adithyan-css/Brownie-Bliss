...
function isPriceAllowed(price) {
    if (selectedPriceFilter === 'under200') return price < 200;
    if (selectedPriceFilter === '200to500') return price >= 200 && price <= 500;
    if (selectedPriceFilter === 'above500') return price > 500;
    return true;
}

function updatePriceFilter() {
    const picker = document.getElementById('priceFilter');
    if (!picker) return;

    selectedPriceFilter = picker.value || 'all';
    ...
    allProductsGrid.querySelectorAll('.product-card').forEach(card => {
        const rawPrice = card.querySelector('.product-price')?.textContent || '';
        const numericPrice = Number(rawPrice.replace(/[^\d.]/g, '')) || 0;
        card.style.display = isPriceAllowed(numericPrice) ? '' : 'none';
    });
}
...
function filterProducts(category, btn) {
    ...
    filtered = filtered.filter(p => isPriceAllowed(Number(p.price) || 0));
...
function addCategoryDefaultToCart(category, fallbackName) {
    const list = products.length ? products : DEFAULT_PRODUCTS;
    const item = list.find(p => p.category === category) || null;
    if (!item) {
        showToast('Item is unavailable right now.');
        return;
    }
    addToCart({
        id: item.id,
...
function addDessertToCart() {
    addCategoryDefaultToCart('desserts', 'Dessert');
}

function addBrownieToCart() {
    addCategoryDefaultToCart('brownies', 'Brownie');
}

function addCookieToCart() {
    addCategoryDefaultToCart('cookies', 'Cookie');
}
